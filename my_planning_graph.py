...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        negated_effectsB = set([~x for x in self.children[actionB]])
        
        return bool(effectsA & negated_effectsB)
    ...
    def _interference(self, actionA, actionB):
        ...
        """
        effectsA = self.children[actionA]
        negated_preconditionsB = set([~x for x in self.parents[actionB]])
        effectsB = self.children[actionB]
        negated_preconditionsA = set([~x for x in self.parents[actionA]])
        
        return bool(effectsA & negated_preconditionsB) or bool(effectsB & negated_preconditionsA)
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        for preconditionA in self.parents[actionA]:
            for preconditionB in self.parents[actionB]:
                if preconditionB in self.parent_layer._mutexes[preconditionA]:
                    return True
    ...
    def _inconsistent_support(self, literalA, literalB):
        ...
        for actionA in self.parents[literalA]:
            for actionB in self.parents[literalB]:
                if actionB not in self.parent_layer._mutexes[actionA]:
                    return False
...
class PlanningGraph:
    ...
    def h_setlevel(self):
        ...
        while not self._is_leveled:
            ...
            if all_goals_appear(layer) and no_mutex(layer):
                return set_level
            set_level += 1
