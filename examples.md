...
(Note: This example function only illustrates the class interface; it is not useful in the project.)
```
class PlanningGraph:
    ...
    def layerloop(self):
...
#### children : dict
A dict mapping from each node in the layer to a set of children nodes in the next layer. For literal layers, the children of each literal node L are the actions in the next layer that includes L as a precondition.
...
This example demonstrates how you could add a method to verify that the child nodes of each literal L in the layer are actions that have L as a precondition. (Note: This example function only illustrates the class interface; it is not useful in the project.)
```
class LiteralLayer:
    ...
